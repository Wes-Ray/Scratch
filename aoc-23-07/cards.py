...
def hand_value(hand : str) -> int:
    ...
    hand_type = 0
    num_wilds = hand.count('J')
    print("num_wilds: ", num_wilds)
    # five of a kind = 6 pts
    if hand.count(hand[0]) == 5:
        hand_type = 6
    # four of a kind = 5 pts
    elif (hand.count(hand[0])+num_wilds == 4) or (hand.count(hand[-1])+num_wilds == 4):
        hand_type = 5
    # full house = 4 pts
    ...
    strength = int(str(hand_type) + card_vals)
    # print("\tstrength: ", strength)
    return strength


def main():
    with open("ezinput2.txt", "r") as file:
